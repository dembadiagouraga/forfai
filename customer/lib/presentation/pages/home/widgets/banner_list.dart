...
class BannerList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<BannerBloc, BannerState>(
      builder: (context, state) {
        return Container(
          color: AppHelpers.getType() == 2 || AppHelpers.getType() == 3
              ? colors.newBoxColor
              : colors.transparent,
                    ...
                    child: SizedBox(
                        ...
                        child: Stack(
                          children: [
                            ...
                            if (state.banners.length > 2 && AppHelpers.getType() == 2)
                              Positioned(
                                bottom: 32.r,
                                left: 32.r,
                                child: SizedBox(
  ...
  Padding _bannerItem(BuildContext context, BannerState state, int index) {
    return Padding(
      padding: EdgeInsets.symmetric(horizontal:  AppHelpers.getType() == 3 ? 0 : 16.r),
      child: ButtonEffectAnimation(
        ...
        child: Container(
          decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(  AppHelpers.getType() == 3 ? 0 : AppConstants.radius.r)),
          child: CustomNetworkImage(
              url: state.banners[index].img  ?? state.banners[index].galleries?.first.path,
              preview: state.banners[index].img != null? null:  state.banners[index].galleries?.first.preview,
              height: double.infinity,
              width: double.infinity,
              radius:  AppHelpers.getType() == 3 ? 0 : AppConstants.radius),
        ),
      ),
    );
  }
