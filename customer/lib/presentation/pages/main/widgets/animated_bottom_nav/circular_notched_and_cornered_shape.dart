...
class CircularNotchedAndCorneredRectangle extends NotchedShape {
  ...
  Path getOuterPath(Rect host, Rect? guest) {
    if (guest == null || !host.overlaps(guest)) {
      if (AppConstants.radius > 0 || AppConstants.radius > 0) {
        double leftCornerRadius =
            AppConstants.radius * (animation?.value ?? 1);
    ...
    double leftCornerRadius = AppConstants.radius * (animation?.value ?? 1);
    double rightCornerRadius = AppConstants.radius * (animation?.value ?? 1);
