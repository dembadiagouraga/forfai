...
class CityRepository extends CoreRepository
{
    ...
    {
        return $model->load([
            'region.translation'  => fn($query) => $query->when($this->language, fn($q) => $q->where('locale', $this->language)),
            'country.translation' => fn($query) => $query->when($this->language, fn($q) => $q->where('locale', $this->language)),
            'translation'         => fn($query) => $query->when($this->language, fn($q) => $q->where('locale', $this->language)),
            'translations',
        ]);
    }
