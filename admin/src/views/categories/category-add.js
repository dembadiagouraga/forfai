...
const CategoryAdd = () => {
  ...
  const onFinish = (values) => {
    ...
    const body = {
      ...
      work: values.work ? 1 : 0,
      keywords: values.keywords.join(','),
      parent_id: values.parent_id?.value,
      ...image?.reduce((acc, cur, index) => {
        acc[`images[${index}]`] = cur?.name;
        return acc;
      }, {}),
    };
    delete body?.image;
  ...
  async function fetchUserCategoryList() {
    const params = { perPage: 100, type: 'main', work: 0 };
    ...
    <Card title={t('add.category')} extra={<LanguageList />}>
      ...
      >
        <Row gutter={12}>
          ...
          {/* <Col span={2}>
            <Form.Item label={t('work')} name='work' valuePropName='checked'>
              <Switch />
            </Form.Item>
          </Col> */}
        </Row>
